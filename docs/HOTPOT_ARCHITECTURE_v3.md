...
- **Camera controls** — every exposure/white-balance/focus/brightness/contrast/saturation/gain/sharpness/hue/backlight-compensation knob the open camera backend exposes (`camera/mjpeg.py`'s `GET /controls.json`/`POST /control`, hit directly from the staff view the same way `/info.json` already is — camera owns this state, core does not proxy it). A control with an auto mode gets an Auto/Manual toggle; Manual unlocks its slider and pins the value, written to `state/camera_settings.json` (the original three fields plus an additive `controls` map for the rest) so it survives a restart the same way §6.6's sweep-and-freeze already does. A "Reset to camera defaults" button undoes a sweep gone wrong: auto-capable controls (white balance/exposure/focus) go back to Auto, and manual-only ones return to `ControlSpec.default` — the real device-reported default on V4L2, a boot-time snapshot (read before this process touched anything) on Windows, since DirectShow has no default-query API; a control with neither known is left alone rather than guessed at. This is the tool for actually finding "the best picture," not just reading numbers — the six read-only stats above answer "what is the camera doing," this answers "make it do something else."
