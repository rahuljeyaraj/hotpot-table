...
Carry it over a bin and the fire changes hands. The ball goes out, the bin's golden ring gives way to a burning one, and it is the bin that's alight now, in a colour of its own. Lift your hand away from the bin and the flame comes back to you, the bin settling behind you into its slow gold breathing again.
...
- **Calories per 100 g**: Button Mushrooms come in at 22, Instant Noodles at 450, which is the single most useful number nobody ever tells you
- **What it actually tastes like**: one honest sentence, written for someone who has never eaten it
...
This is the number I never had in Singapore. It means you can take 60 g of the $5.00 eel because you can see it's costing you $3.00, and you can be generous with the $1.00 potato because you can see that it barely registers. The expensive decisions and the cheap ones stop feeling the same.
...
- **Classic Mala Broth.** *Very spicy.* Numbing and fiery, built on Sichuan pepper and chilli. The boldest broth here.
- **Mushroom Vegan Broth.** *Not spicy.* Deep and savoury with no meat at all. Earthy rather than rich.
- **Collagen Bone Broth.** *Not spicy.* Milky and slow simmered. Rich on the tongue but completely mild.
...
Everything past this point is the engineering. If you want to build one, follow sections 5, 6 and 7. If you want to understand how it works, section 8 covers the architecture and the fire.
...
D4  / GPIO5     bin 1    far centre-left
D10 / GPIO9     bin 2    far centre-right
D9  / GPIO8     bin 3    far right
D5  / GPIO6     bin 4    near left
D6  / GPIO43    bin 5    near centre-left
D8  / GPIO7     bin 6    near centre-right
...
Mount the projector on the wall, right against the ceiling, aimed straight down at the table. Set it back far enough that the image covers the full 1524 mm width.

The camera rides on the projector. A wooden batten is fixed to the projector's mount and sticks out from it. Zip tie the camera to the end of that batten, looking down at the same table. On this build the camera hangs upside down — 180 degrees.

Square the image with the projector's own 4D keystone. Nothing else is needed for that: the app just draws a plain 1920 x 1080 rectangle and sends it straight to the lens.

Cabling:

- HDMI to the projector
- One USB extension to the camera
- One USB extension to the XIAO
- The camera needs a USB 3 port on the host; the XIAO doesn't
...
# built Aug 28 2026 11:04:12
# probe: all 8 cells reporting
# ready
...
What comes out is a single point, the index fingertip, put through the camera homography into projector pixels and smoothed so per-frame jitter never reaches the cursor. The table follows one hand at a time.
...
- **Full nutrition, not just calories.** The info box already gives calories per 100 g and a veg/non-veg/egg flag for whichever bin a hand is over. The next step is macros (protein, carbs, fat) and key micronutrients per ingredient, then totalled live across the whole bowl - the same weight-driven subtraction that already prices it, applied to nutrition instead of cost, so a diner leaves knowing not just what they spent but what they ate.
