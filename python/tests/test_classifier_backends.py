...
class TestEiCppBackend(unittest.TestCase):
    ...
    def build(self, run, *, binary_exists=True):
    ...
    def test_the_crop_is_resized_and_converted_to_rgb_before_writing(self):
        # backend_ei.INPUT_WIDTH/HEIGHT must be what actually gets written
        # — a size mismatch is exactly what segfaulted tools/eim_cpp's CLI
        # before it grew its own guard (main.cpp's own comment on that).
        import struct
        seen = {}
        ...
        self.assertEqual((seen["w"], seen["h"]),
                         (backend_ei.INPUT_WIDTH, backend_ei.INPUT_HEIGHT))
        self.assertEqual(seen["nbytes"],
                         backend_ei.INPUT_WIDTH * backend_ei.INPUT_HEIGHT * 3)
