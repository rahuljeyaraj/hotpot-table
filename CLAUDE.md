...
### Build item 6, taken out and replaced (2026-08-14): fire ring → spark shower
Rig confirmation on the obstacle fix above never happened — three new
reports arrived first, developer's own words: "Current state: bins are
now obstacles in the fluid sim — but that didn't solve the core problem.
Selected bin still doesn't look unique." Plus two concrete bugs:

1. **Bin 5 false-positive**: highlight fires with the hand ~3cm outside
   its right edge. **Not an oF bug, and no code change here fixes it** —
   traced to `core/bin_grid.py`'s own documented design: `hover.bin_under`
   (`core/hover.py`) hit-tests against `core.camera_grid`, the CAMERA
   grid, while everything oF draws (`state.bins[].rect`, this class's
   `binRectPx`) comes from `core.projector_grid` — two grids, dragged
   independently by a human looking at two different things (a camera
   crop vs. the real projected light), never derived from each other.
   `bin_grid.py`'s module docstring already names this exact failure
   mode: "if a human has set the two grids differently the highlight
   appears slightly off the hand. That is inherent in two independently-
   authored grids." Column 1 (bins 1 and 5 share `v_lines[2]`/`v_lines[3]`,
   `BinGrid.rects()`) needs its camera-grid right edge re-nudged inward
   on whatever tool drives `set_grid` (`core/main.py::_handle_set_grid`)
   — a calibration action, not a patch. Left as a note in `UiLayer.cpp`'s
   `update()` for the next time a highlight looks spatially wrong.
2. **Bin 5 fire too strong**: left hovering, then moving the hand away,
   the residual flame "blows downward, like air is pushing down from
   above." Consistent with the same buoyancy/dissipation mechanism the
   earlier fix targeted (a lingering temperature field driving airflow
   after the density puff itself has faded) — but developer's fix
   direction supersedes chasing it further: drop fire from bin
   highlighting entirely.

**New direction, developer's own design**: replace the fire-ring
highlight with a spark shower — particles that spawn on the bin's ring
circumference (the halo's own inner radius, `kSparkRingMarginPx` =
`kHaloMarginPx`), fire a short, fizzling distance, and fall under real
gravity, "unlike fire." "Ring stays clean/visible — sparks are a halo of
motion around the edge," not a filled band the way the fire ring's
injected density was.

Implementation is pure `UiLayer` — no `FluidLayer` API at all. This was
the biggest simplification of the four files touched: the fire ring was
the only reason `FluidLayer::update()` took a per-bin argument in the
first place, and ballistic sparks (`pos += vel*dt`, `vel.y += gravity*dt`)
need nothing from `ftFluidFlow` — no shared field, so one bin's motion
can never bleed into another's the way a shared density/temperature
buffer could. Removed outright, not deprecated: `FluidLayer::FireRing`,
its injection block and `drawRoundedBand` helper in `FluidLayer.cpp`, and
`UiLayer::FireEmitter`/`fireEmitters()`. `BinTween::fire` renamed to
`spark` (still the same 350ms crossfade spring, still what `drawHalo`
fades OUT by — doc §6's "gold halo crossfades out as the highlight
crossfades in" relationship is unchanged, only what fades IN changed).

New in `UiLayer`: `Spark{pos, vel, age, lifespanS}`, one
`std::vector<Spark>` and one fractional spawn accumulator per bin.
`updateSparks()` (called from `update()`, right after the spark spring
steps) ages/culls existing sparks, then spawns new ones at
`kSparkSpawnRatePerSec * intensity` — a point on the ellipse inscribed in
`binRectPx(i)` inflated by the halo's own margin, launched either
radially outward or, `kSparkTangential = true`, tangent to the ring.
`drawSparks()` draws each as a small filled circle (`kSparkRadiusPx`,
2.5px — a filled shape, not an `ofPath` stroke, same glLineWidth-cap
reason every other ring in this file avoids one), colour lerping hot
(255,210,120) to cold (210,70,30) and alpha fading quadratically over its
`0.18–0.35s` lifespan — same falloff shape `drawHalo`'s own bands use, so
the crossfade doesn't trade one easing curve for a visibly different one
mid-transition.

**Radial vs. tangential is the one call left open, developer's own
words**: "radial (shoot outward) vs tangential (race around ring) — TBD
which reads better." Shipped `kSparkTangential = false` (radial) for the
first look — "sparks fire outward" was the plainer of the two
descriptions given, and gravity has an obvious thing to fight against on
a radial launch. Flip the one `const bool` in `UiLayer.cpp` and rebuild
to compare tangential; no other constant changes either way.

Kept, deliberately: bins as fluid obstacles (the section above) — that
still holds for the ambient hand-driven flame (a hand's own trail should
still curl around a bin edge rather than cross into it), independent of
however the selected bin gets highlighted.

Full rebuild, msbuild Debug x64, 0 errors, same 2 pre-existing warnings.
`run.py --stop` / rebuild / `run.py` again — camera/core/voice/
classifier/tracker all reached ready, `of`'s StateLink reconnected, no
new errors. **Not yet seen on the rig.** Next: does the spark shower read
as "this bin is selected" where the fire ring didn't; does radial read
right or does it need the tangential flip; and separately (a calibration
task, not a code one) — someone needs to re-nudge bin 5's camera-grid
column on the grid-setup tool.
